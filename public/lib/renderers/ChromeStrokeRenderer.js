    ...
    shading() {
        return /* glsl */`
            ...
            void main() {
                ...
                // Horizon split, wide enough that the bead's relief reads as a smooth
                // sweep across it rather than a hard edge broken into facets.
                float horizon = smoothstep(-0.22, 0.22, -r.y);
                vec3 env = mix(uGround, uSky, horizon);
                // A second, dimmer band keeps the dark side from reading as flat paint.
                env = mix(env, uSky, smoothstep(0.4, 1.0, abs(r.x)) * 0.25);
