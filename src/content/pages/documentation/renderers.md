...
Paths derived from the drawn path, each drawn with the brush renderer. Six modes: `spiral` (the tip circles while its center moves along the path, one continuous coil), `entangled` (copies of the path offset by seeded low-frequency waves, their endpoints pulled back toward the base), `scattered` (short strokes copying small segments of the path, moved sideways by a seeded offset), and three crossing waves: `wiggle` (its wavelength tightening from loose at the start to tight at the end), `wiggle-even` (the same wave at a constant wavelength), and `wiggle-u` (a constant wavelength whose lobes are U turns, not sine humps). The spiral, entangled, and scattered sizes follow the width through `reach`; the wiggles instead read the drawn width as the wave's amplitude, so widening the stroke grows the whole shape, and take their wavelength as an absolute size in `wavelength` (world units). The brush line that traces a wiggle is not the drawn width but a thickness derived from the amplitude and wavelength, clamped to 2 to 6 pixels and then scaled by `density`, so it stays balanced to the shape unless the caller thins or thickens it (the tightening wiggle floors its tight end at the amplitude). Every count (the spiral's turns, the wiggle's crossings, the scatter's strokes) follows the path's length, so the pattern keeps its spacing as the stroke grows instead of crowding a short stroke and stretching a long one. The spiral, entangled, and scattered width is capped at 0.03 world units for the derivation, the range the formulas are calibrated for. Takes `mode`, `colorA`, `colorB` (alternated between sub-strokes), `reach` (how far the spiral, entangled, and scattered paths stray, in widths), `cycle` (the spiral's advance per turn, as a multiple of its radius, so at 1 the loops half-overlap and near 2 they sit beside each other), `wavelength` (the wiggle's wavelength, in world units; the drawn width is its amplitude), and `density` (a multiplier on the wiggle's line width). The generators are documented on the Path Effects page.
<div class="jp">描かれたパスから導いたパスを、それぞれbrushレンダラで描きます。6つのモードがあります。`spiral`（中心がパスに沿って進むあいだ、先端が円を描く、一本の連続したコイル）、`entangled`（シード付きの低周波の波でずらされたパスの複製で、端点は元のパスへ引き戻されます）、`scattered`（元のパスの一部を写し取り、シード付きのオフセットで横へ移動する短いストローク）、そして3つの横切る波です。`wiggle`（波長が始めのゆるいものから終わりのきついものへと詰まります）、`wiggle-even`（同じ波を一定の波長で）、`wiggle-u`（一定の波長で、ローブがサインの山ではなくU字のターンになります）。spiral・entangled・scatteredの大きさは`reach`を通して幅に従います。ウィグルは描かれた幅を波の振幅として読むため、ストロークを太くすると形全体が大きくなり、波長は`wavelength`で絶対的な大きさ（ワールド単位）として取ります。ウィグルをなぞる筆の線は描かれた幅ではなく、振幅と波長から導かれて2〜6ピクセルに収められ、さらに`density`で拡縮された太さなので、呼び出し側が細くも太くもしない限り形に対して釣り合いを保ちます（詰まるウィグルはきつい端を振幅で下限を設けます）。本数（スパイラルの回転、ウィグルの横切り、散布のストローク）はすべてパスの長さに従うため、短いストロークで詰まり長いストロークで伸びることなく、パターンは間隔を保ちます。spiral・entangled・scatteredの幅は0.03ワールド単位で頭打ちになります。式が調整された範囲だからです。`mode`、`colorA`、`colorB`（サブストロークごとに交互）、`reach`（spiral・entangled・scatteredのパスが離れる距離、幅単位）、`cycle`（スパイラルの1回転あたりの進み、半径の倍数。1でループは半分重なり、2付近で隣り合います）、`wavelength`（ウィグルの波長、ワールド単位。描かれた幅が振幅です）、`density`（ウィグルの線の太さの倍率）を受け取ります。生成器の説明はPath Effectsページにあります。</div>
...
<li><code>PatternedFillRenderer</code> — a flat fill whose interior is a repeating pattern, evaluated over world space so it holds still wherever the shape lands, in one of three: <code>dots</code> (circles on a hexagonal lattice, each a random color from <code>colors</code> with a small seeded offset from its lattice point, over <code>background</code>), <code>squares</code> (a grid turned 45 degrees, a colored square inset in each cell, no offset), and <code>waves</code> (the first two of <code>colors</code> in undulating bands, after Bridget Riley). Takes <code>pattern</code>, <code>colors</code>, <code>background</code>, <code>scale</code> (cell or band size), <code>jitter</code>, <code>inset</code>, <code>waveAmp</code>, and <code>waveFreq</code>.<br /><span class="jp">内部が繰り返しのパターンになった平坦な塗りで、パターンはワールド座標で評価されるため、形がどこに置かれても止まったままです。3種類あります。<code>dots</code>（六角格子の上の円。それぞれ<code>colors</code>からランダムな色を取り、格子点からシードで少しずらされ、<code>background</code>の上に置かれます）、<code>squares</code>（45度回した格子で、各セルの中に色付きの正方形を内側に描きます。ずれなし）、<code>waves</code>（<code>colors</code>の最初の2色がうねる帯になります。ブリジット・ライリーにならったもの）。<code>pattern</code>、<code>colors</code>、<code>background</code>、<code>scale</code>（セルまたは帯の大きさ）、<code>jitter</code>、<code>inset</code>、<code>waveAmp</code>、<code>waveFreq</code>を取ります。</span></li>
...
<li><code>StoneBlobRenderer</code> — the blob as stone. Rock folds its noise into creases with mottled color patches, and its boundary breaks on the same crags; marble runs thin noise-warped veins over a near-white glossy ground; sand mounds on a low-frequency swell with a fine per-pixel normal jitter as grain over it, occasional glints, and an edge that dissolves into loose grains. Takes <code>mode</code>, <code>colorB</code>, <code>relief</code>.<br /><span class="jp">石としてのブロブ。岩はノイズをひだに折り返し、色を斑に散らし、境界も同じ岩肌で割れます。大理石は白に近い光沢のある地の上に、ノイズでゆがめた細い脈を走らせます。砂は低周波のうねりで表面を盛り上げ、その上に細かいピクセルごとの法線の揺らぎを粒として乗せ、ときおり粒をきらめかせ、縁はばらけた粒に崩れます。</span></li>
</ul>
</div>

## 3D strokes

Strokes built from 3D shapes around the spine, lit and baked onto the canvas like any other mark. `Stroke3DRenderer` is the base: the spine gains depth from a seeded wave whose wavelength tracks the stroke's width (so a wide tube snakes as gently as a thin one rather than rippling faster than it is thick), so the mark reads as an object lying over the canvas, and the shape rotates around the spine by an angle keyed to the distance from one end. A seeded offset also pushes the shape slightly off the spine, in a direction that rotates with the same angle, so the mark orbits the spine along its length. By default the angle keys on distance from the start, so the drawn part holds still as the stroke grows; the solid strokes key it on the end (`spinFromTip`), eased so the spin fades over `spinReach` behind the tip, and the newest solids turn as the stroke grows while older ones settle to a fixed angle. The mark floats about 100 CSS pixels over the canvas, lit from the upper left (60 degrees down from the screen's up axis, 30 degrees to the left of the camera). A tube cannot bend tighter than its own radius without the inner wall crossing itself and tearing the surface, so the centerline is eased to that limit before the rings are built, by averaging each point over its neighbors within about a radius of arc: a thin tube keeps its corners, a fat one rounds them the way a real tube of that thickness would. The ring's reach toward a bend's center is still clamped as a backstop, and any residual fold is drawn double-sided with the inward normal flipped, so it reads as a crease rather than a hole. Both members take `depth`, `twist`, `zBase`, and `wander` (the offset's amplitude).
<div class="jp">スパインの周りの3D形状から作られ、他の筆跡と同じように照らされてキャンバスに焼き込まれるストロークです。基底クラスは`Stroke3DRenderer`です。スパインは、波長がストロークの幅に追従するシード付きの波から深さを得るため（幅の広いチューブは、太さより速く波打つのではなく、細いものと同じくらいゆるやかにうねります）、筆跡はキャンバスの上に置かれた物体として読めます。また、形は一方の端からの距離に応じた角度でスパインの周りを回ります。シード付きのオフセットも形をスパインからわずかに押し出し、その方向は同じ角度で回転するため、筆跡はスパインの周りを長さに沿って周回します。既定では角度は始点からの距離に基づくため、描かれた部分はストロークが伸びても動きません。立体のストロークは終端に基づき（`spinFromTip`）、先端の背後の`spinReach`にわたって回転が弱まるようにならされるため、先端付近の新しい立体はストロークが伸びるにつれて回り、古い立体は一定の角度に落ち着きます。筆跡はキャンバスの約100CSSピクセル上に浮かび、左上からの光（画面の上方向から60度、カメラから左へ30度）に照らされます。チューブは自身の半径より急には曲がれず、無理に曲げると内壁が自分自身と交差して面が裂けます。そこでリングを組む前に、各点をおよそ半径ぶんの弧の範囲の近傍で平均して中心線をその限界までならします。細いチューブは角を保ち、太いチューブはその太さのチューブが実際にそうなるように角を丸めます。曲がりの中心に向かうリングの届く距離も予備として制限され、残った折れは両面描画で内向きの法線を反転して描かれるため、折れは穴ではなく折り目として読めます。どちらのメンバーも<code>depth</code>、<code>twist</code>、<code>zBase</code>、<code>wander</code>（オフセットの振幅）を取ります。</div>
...
<li><code>SolidStrokeRenderer</code> — a chain of flat-shaded 3D solids, in three shapes (<code>tetra</code>, <code>box</code>, <code>cone</code>). Each takes a seeded size, side, and orientation, and is thrown to one side of the spine so the chain fills a band. The step between neighbors is their two circumradii plus <code>spacing</code> of their sum, so tighter spacing packs more of them. Color is chosen per face group, so a cube's square and a cone's base are each one flat color while a tetrahedron's every face is its own, and the rotation keys on the tip and fades behind it, so the solids near the tip turn as the stroke grows while older ones settle. Takes <code>shape</code>, <code>colors</code>, <code>spacing</code>, <code>spread</code> (how far solids are thrown to the side, in half-widths), and <code>spinReach</code> (the distance behind the tip over which the spin fades).<br /><span class="jp">フラットに陰影付けされた3Dの立体の連なりで、<code>tetra</code>・<code>box</code>・<code>cone</code>の3つの形があります。各立体はシードによる大きさ・左右・向きを取り、スパインの片側へ振り出されるため、連なりは帯を埋めます。隣どうしの間隔は2つの外接半径にその和の<code>spacing</code>倍を足したものなので、間隔を詰めるほど数が増えます。色は面のグループごとに選ばれるため、立方体の正方形と円錐の底面はそれぞれ一色、四面体は各面が自分の色になります。回転は先端に基づき、その背後で弱まるため、先端付近の立体はストロークが伸びるにつれて回り、古いものは落ち着きます。<code>shape</code>、<code>colors</code>、<code>spacing</code>、<code>spread</code>（立体を横へ振り出す距離、半幅単位）、<code>spinReach</code>（先端の背後で回転が弱まる距離）を取ります。</span></li>
</ul>
