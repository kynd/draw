...
Strokes built from 3D shapes around the spine, lit and baked onto the canvas like any other mark. `Stroke3DRenderer` is the base: the spine gains depth from a seeded wave of arc length, so the mark reads as an object lying over the canvas, and the shape rotates around the spine by an angle keyed to the distance from the stroke's end, so a growing stroke visibly turns while it is drawn. The rotation is the one deliberate exception to prefix stability; the depth wave keys on distance from the start and holds still. The mark floats about 100 CSS pixels over the canvas and casts a drop shadow onto it: the geometry flattened, pushed down the screen as the light (from the top, 30 degrees above the camera) would throw it, black at one flat tenth of opacity however much of the mark overlaps. Both members take `depth`, `twist`, and `zBase`.
<div class="jp">スパインの周りの3D形状から作られ、他の印と同じように照らされてキャンバスに焼き込まれるストロークです。基底クラスは`Stroke3DRenderer`です。スパインは弧長のシード付き波から深さを得るため、印はキャンバスの上に置かれた物体として読めます。形はまた、ストロークの終端からの距離に応じた角度でスパインの周りを回転するため、伸びていくストロークは描いている間、目に見えて回ります。この回転はプレフィックスの安定性に対する唯一の意図的な例外です。深さの波は始点からの距離に基づき、動きません。印はキャンバスの約100CSSピクセル上に浮かび、その上に落ち影を落とします。ジオメトリを平らにし、光（上から、カメラの30度上）が投げる位置へ画面の下方向へずらしたもので、印がどれだけ重なっても平坦な、黒の不透明度0.1です。どちらのメンバーも<code>depth</code>、<code>twist</code>、<code>zBase</code>を取ります。</div>
