...
<div class="jp">`singleCoverage`は、印ごとに各ピクセルを一度だけ陰影付けします。リボンが自分の上に重なる場所では、半透明のマテリアルは二度合成されて折り目のように濃くなってしまいます。印はカバレッジレイヤに登録され、レイヤは印を単独でオフスクリーンターゲットにMAXブレンディングで描画します。何度覆われても最も強いひとつだけが残り、ターゲットはキャンバスへちょうど一度だけ合成されます。しきい値がないため、継ぎ目の生まれる場所がありません。コストは、印が生きている間だけの、フラグ付きの印ごとの画面サイズの合成ひとつです。焼き込みが印を平坦にするからです。分割された断片は別々の印として重なり合います。ドライメディアはこれをオンにします。</div>
