...
<div class="jp">スパインの周りの3D形状から作られ、他の筆跡と同じように照らされてキャンバスに焼き込まれるストロークです。基底クラスは`Stroke3DRenderer`です。スパインは弧長のシード付き波から深さを得るため、筆跡はキャンバスの上に置かれた物体として読めます。また、形はストロークの終端からの距離に応じた角度でスパインの周りを回るため、伸びていくストロークは描かれている間、目に見えて回転します。シード付きのオフセットも形をスパインからわずかに押し出し、その方向は同じ角度で回転するため、筆跡は回るだけでなくスパインの周りを周回します。この回転はプレフィックスの安定性に対する唯一の意図的な例外です。深さの波とオフセットの振幅は始点からの距離に基づき、動きません。筆跡はキャンバスの約100CSSピクセル上に浮かび、左上からの光（画面の上方向から60度、カメラから左へ30度）に照らされます。チューブの太さより急な曲がりでは、曲がりの中心に向かうリングの届く距離が制限されるため、隣り合うリングが互いを通り抜けて面が折れることはありません。どちらのメンバーも<code>depth</code>、<code>twist</code>、<code>zBase</code>、<code>wander</code>（オフセットの振幅）を取ります。</div>
