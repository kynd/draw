...
`singleCoverage` dedupes a mark's self-overlap where a bend folds the ribbon over itself: a translucent material would composite the fold twice and darken into creases, so depth write with a strict less test makes the first fragment claim the pixel. A genuine crossing should still layer, and arc length tells the two apart: the geometry's z climbs in a staircase of arc windows, so a fold (neighbors, same window) stays at equal depth and dedupes, while a crossing (a much later window) sits closer and composites over the earlier pass. The dry media turn it on.
<div class="jp">`singleCoverage`は、曲がりでリボンが自分の上に折り重なる場所の自己重なりを、重複なく描きます。半透明のマテリアルは折りを二度合成して折り目のように濃くしてしまうため、深度書き込みと厳密なlessのテストにより、最初のフラグメントがピクセルを取ります。本当の交差は重なるべきで、弧長がその2つを区別します。ジオメトリのzは弧長の窓ごとの階段で上がるため、折り（隣同士、同じ窓）は同じ深度に留まって重複が除かれ、交差（ずっと後の窓）はより手前にあって、先に描かれた部分の上に合成されます。ドライメディアはこれをオンにします。</div>
