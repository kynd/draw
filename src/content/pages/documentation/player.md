...
<div class="jp">エンジンはインターフェースを持ちません。面には、コンストラクタに渡す3つのコールバック、`feed`（描画サイクルのfeed）、`applyRecord`（ひとつの記録のツールと色を戻す）、`clear`（面を背景の指定にクリアする）を通して触れます。`canvas`はビデオのためにキャプチャされます。</div>
...
<div class="jp">`setData({ background, records })`はログを受け取り、それまでのものを置き換えます。`play({ pointsPerFrame, onDone })`は背景にクリアしてから記録を再生し、再生が始まったかどうかを返します。`finish()`は実行中の再生を最終状態まで進めます。`record({ filename, onDone })`はキャンバスをキャプチャしながら再生し、ビデオを保存します（ブラウザがエンコードできる場合はmp4、そうでなければwebm）。状態は`hasData`、`playing`、`recording`で確認できます。</div>
