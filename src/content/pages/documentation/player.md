...
<div class="jp">`setData(data)`はログを受け取り、それまでのものを置き換えます。`play({ pointsPerFrame, strokeWaitMs, instantInitial, onDone })`は現在位置からアニメーションし、`release`の印が付いた記録のあとで`strokeWaitMs`だけ休みます。`instantInitial`（既定）では、`initial`の印が付いた記録はアニメーションせず一瞬で置かれます。`pause()`は描かれたものを残したまま止まり、`resume()`は続きを再生します。`seek(index)`は、その数の記録が描かれた状態まで移動します。`next()`と`prev()`は1記録分のシーク、`rewind()`はクリアされた背景まで戻り、`finish()`は最後まで進めます。状態は`position`、`length`、`playing`、`recording`で確認でき、`on(event, fn)`で`'step'`、`'play'`、`'pause'`、`'end'`、`'record-start'`、`'record-end'`を購読できます。</div>
